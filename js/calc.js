...
/**
 * Repartition des series MQ pour n pilotes engages.
 *
 ...
 *
 * @param {number} n     — nombre de pilotes
 * @param {number} max   — taille maximale d'une serie
 * @param {string} [mode] — 'ffsa' (defaut) | 'fia_even'
 * @returns {number[]} tailles des series, dans l'ordre
 */
export function computeSeriesSizes(n, max, mode) {
  max = max || 5;
  if (n <= 0) return [];
  if (n <= max) return [n];
